...
$("#btnScheduleName").on("click", function(e) {
	...
	*/
	...
	if(category_id == "server"){
	...
	} else if(category_id == "pc"){
		// 개인정보유형 아이디 검색
		$.ajax({
			type: "POST",
			url: "/search/selectScanDataTypes",
			async : false,
			data : {
				std_id : $('#std_id').val()	
			},
		...
		$.each($("#pc_tree").jstree("get_checked",true),function(){
			...
			if(type == 1){
				...
				$.each(datatypes, function(index, item){
					if(item.ap_no == ap_no){
						datatype = item.datatype_id;
					}
				});
